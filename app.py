...
if check_auth():
    ...
    if dati:
        if scelta == "Lista":
            ...
            st.title("Visualizzazione 3D Avanzata")

            # --- GRAFICO A BARRE 3D ---
            # Questo grafico simula l'effetto profondità richiesto
            fig3d = go.Figure()

            for idx, i in enumerate(dati):
            ...
            # --- GRAFICO A TORTA ---
            st.subheader("Ripartizione Capitale")
            fig_pie = px.pie(dati, values='Val', names='Nome', hole=0.4)
